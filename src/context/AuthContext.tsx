 ...
 *   - Demo account: seeded automatically for instant one-click demo
 *     sign-in (see DEMO_ACCOUNT below)
 *
...
/**
 * Built-in demo account — seeded automatically on first load so anyone
 * can explore the app without signing up. The login page shows a
 * one-click "Continue as Demo User" button using these credentials.
 */
export const DEMO_ACCOUNT = {
  name: "Demo User",
  email: "demo@apexstrategy.app",
  password: "demo1234",
};

const DEMO_USER_ID = "user-demo-apex";
...
/** Ensure the built-in demo account exists (idempotent, no-op if present). */
function seedDemoUser(users: StoredUser[]): StoredUser[] {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Load session on mount (and make sure the demo account exists)
  React.useEffect(() => {
    try {
