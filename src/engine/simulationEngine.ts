  ...
  const teamResults: TeamRoundResult[] = state.teams.map((team) => {
    ...
    return {
      ...
      alerts: [{
        ...
        message: "Make decisions across R&D, Marketing, Operations, and Finance, then process the next round.",
        teamId: team.id,
        category: "system",
      }],
    };
...
function computeTeamRoundResult(
  ...
  // Total team market share = team units sold / total market demand
  // (across ALL segments — the true overall share, so the leaderboard
  // always matches the doughnut chart's readings exactly)
  const allSegmentDemand = state.segments.reduce((s, sg) => s + sg.totalDemand, 0);
  const teamTotalUnits = productResults.reduce((s, r) => s + r.unitsSold, 0);
  const teamShare = (teamTotalUnits / Math.max(allSegmentDemand, 1)) * 100;
 ...
 * calculator on R&D / Marketing / Operations / Finance tabs
