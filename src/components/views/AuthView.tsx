...
export function AuthView() {
  ...
  // One-click demo sign-in — lets anyone explore the app instantly
  // without creating an account.
  const handleDemoSignIn = () => {
    setError(null);
    setSubmitting(true);
    setTimeout(() => {
      const result = signIn(DEMO_ACCOUNT.email, DEMO_ACCOUNT.password);
      setSubmitting(false);
      if (!result.ok) {
        setError(result.error || "Demo sign-in failed");
      } else {
        toast("Signed in as Demo User — explore freely!", "success");
      }
    }, 350);
  };

  return (
    <div className="min-h-screen apex-hero-bg flex flex-col">
      <div className="apex-grid grid lg:grid-cols-2 flex-1">
        {/* ── Left: brand hero ── */}
                  ...
                  {mode === "signin" && (
                    <>
                      ...
                      <p className="text-center text-[10px] text-muted-foreground mt-2">
                        Demo credentials:&nbsp;
                        <span className="font-mono text-foreground/80">{DEMO_ACCOUNT.email}</span>
                        &nbsp;/&nbsp;
                        <span className="font-mono text-foreground/80">{DEMO_ACCOUNT.password}</span>
                      </p>
                    </>
                  )}
      ...
      <footer className="border-t border-border/40 bg-background/50 backdrop-blur-sm">
        <div className="mx-auto max-w-6xl px-6 py-4 flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 text-xs text-muted-foreground">
          <span>ApexStrategy Enterprise · Created by</span>
          <span className="font-semibold text-foreground">Magesh Kanna S</span>
