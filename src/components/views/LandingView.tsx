      ...
      <div className="apex-grid min-h-screen">
        ...
        <div className="border-b border-border/40 bg-background/60 backdrop-blur-sm">
          <div className="mx-auto max-w-6xl px-6 py-4 flex items-center justify-between">
            ...
            {/* Right side: badge + account menu (sign out to switch accounts) */}
            <div className="flex items-center gap-3">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    className="flex items-center gap-2 rounded-full pl-1 pr-2.5 py-1 border border-border/60 bg-card/60 hover:border-primary/40 transition-colors"
                    title={user?.name ? `Signed in as ${user.name}` : "Account"}
                  >
                    <span
                      className="grid place-items-center h-7 w-7 rounded-full text-white font-bold text-xs"
